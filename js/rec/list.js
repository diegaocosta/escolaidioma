...
function montar()
{
    ...
    for (let i of lista)
    {
        ...
        t += "<a href='recVer.php?" + i.id + "'>Ver</a>"
        t += "</div>"
    }
...
function ler()
{
    lista = getJSON("../api/rec/listSimples.php")
    montar()
}
function pesquisar()
{
    ...
    lista = getJSON("../api/rec/pesq.php?" + d1 + "&" + d2)
    montar()
}
function gerarPdf()
{
    ...
    if (d1 != "" && d2 != "")
    {
        comeco.push(
            {
                text: ["Receitas entre a data " + formatarData(d1) + " e " + formatarData(d2)], style: "header", fontSize: 14
            }
        )
    }
    else
